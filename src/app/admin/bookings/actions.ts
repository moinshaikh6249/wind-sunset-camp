...
/**
 * Verifies the user's ID token to ensure they are an admin.
 * Throws an error if the token is missing or if the user is not an admin.
...
async function verifyAdmin(idToken: string) {
    ...
    try {
        ...
        // Re-throw a more specific error for the client
        throw new Error(error.message || 'Could not verify administrator status.');
    }
}
...
export async function approveBooking(idToken: string, userId: string, bookingId: string) {
    ...
    try {
        ...
        throw new Error('An unexpected error occurred while approving the booking.');
    }
}
